...
async def fetch_live_telemetry() -> TelemetryResponse:
    async with httpx.AsyncClient() as client:
        ...
        results = await asyncio.gather(
            ...
            ping_url(client, settings.PORTFOLIO_HEALTH_URL),
    ...
    portfolio_up, p_lat = results[2] if not isinstance(results[2], Exception) else (False, 0)

    # Real calculations
    nodes_up = sum(1 for status in [kukufiti_up, mmg_up, portfolio_up] if status)
    total_nodes = 3
